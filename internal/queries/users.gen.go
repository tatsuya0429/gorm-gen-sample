...
func newUser(db *gorm.DB, opts ...gen.DOOption) user {
	...
	_user.Todos = userHasManyTodos{
		db: db.Session(&gorm.Session{}),

		RelationField: field.NewRelation("Todos", "models.Todo"),
		User: struct {
			field.RelationField
			Todos struct {
				field.RelationField
			...
			RelationField: field.NewRelation("Todos.User", "models.User"),
			Todos: struct {
				field.RelationField
			}{
				RelationField: field.NewRelation("Todos.User.Todos", "models.Todo"),
			},
		},
	}
...
func (a userHasManyTodos) Where(conds ...field.Expr) *userHasManyTodos {
	if len(conds) == 0 {
		return &a
	}
...
func (a userHasManyTodos) WithContext(ctx context.Context) *userHasManyTodos {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a userHasManyTodos) Session(session *gorm.Session) *userHasManyTodos {
	a.db = a.db.Session(session)
	return &a
}

func (a userHasManyTodos) Model(m *models.User) *userHasManyTodosTx {
	return &userHasManyTodosTx{a.db.Model(m).Association(a.Name())}
}

type userHasManyTodosTx struct{ tx *gorm.Association }

func (a userHasManyTodosTx) Find() (result []*models.Todo, err error) {
	return result, a.tx.Find(&result)
}

func (a userHasManyTodosTx) Append(values ...*models.Todo) (err error) {
...
func (a userHasManyTodosTx) Replace(values ...*models.Todo) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a userHasManyTodosTx) Delete(values ...*models.Todo) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a userHasManyTodosTx) Clear() error {
	return a.tx.Clear()
}

func (a userHasManyTodosTx) Count() int64 {
	return a.tx.Count()
}
...
type IUserDo interface {
	...
	FindByID(id int64) (result models.User, err error)
	FindByUserId(userId int64) (result []models.User, err error)
	FindAll() (result []models.User, err error)
}

// SELECT * FROM @@table WHERE id = @id
func (u userDo) FindByID(id int64) (result models.User, err error) {
...
func (u userDo) FindByUserId(userId int64) (result []models.User, err error) {
	var params []interface{}
...
// SELECT * FROM @@table
func (u userDo) FindAll() (result []models.User, err error) {
	var generateSQL strings.Builder
